...
# USER CONFIGURATION:
# Configure Python module names here
set python_module_primary = "Python3/3.11.1"
set python_module_fallback = "Python3/3.11.8_gpu_torch251"

# Module load Python3
set tmp_file = "module.tmp"
(module load $python_module_primary) >&! $tmp_file
set output = `cat $tmp_file`
if ("$output:q" =~ "*ERROR*" || "$output:q" =~ "*failed*") then
    module load $python_module_fallback
    echo "Module load $python_module_fallback"
else
    module load $python_module_primary
    echo "Module load $python_module_primary"
endif
rm -f $tmp_file
...
if ($new_env == 1) then
    eval "pip3 install --upgrade pip setuptools wheel"
